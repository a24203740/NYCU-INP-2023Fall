			...
			if (FD_ISSET(fd, &readyFD))
			{
				...
				else if (fd == tunFD)
				{
					...
					while (offset < nread)
					{
						...
						int nwrite = write(routeClientFD->second, message + curOffset, length);
						if(nwrite < 0)
						{
				...
				else
				{
					...
					while (offset < nread)
					{
						...
						{
							nwrite = write(tunFD, message + curOffset, length);
							if(nwrite < 0)
							{
						...
						nwrite = write(routeClientFD->second, message + curOffset, length);
						if(nwrite < 0)
						{
							perror("[server] write to client");
							continue;
...
int tunvpn_client(const char *server, int port)
{
	...
	char assignedIP[16];
	read(serverFD, &assignedIP, sizeof(assignedIP));
	...
	// configure tun interface
	{
		...
		ifreq_set_addr(socketFDForTun, dev, inet_addr(assignedIP));
		...
		for (int fd = 0; fd <= maxFD; fd++)
		{
			if (FD_ISSET(fd, &readyFD))
			{
				...
				while (offset < nread)
				{
					...
					{
						nwrite = write(serverFD, message + curOffset, length);
						if(nwrite < 0)
						{
