...
bool Client::setupSockaddr()
{
    ...
    {
        std::cout << "Error resolving IP to addrinfo!" << std::endl;
        return false;
    }
...
bool Client::setupSockaddr(const char* ip)
{
    bzero(&serverSockAddr, sizeof(serverSockAddr)); // set all bytes to 0
    serverSockAddr.sin_family = AF_INET; 
    int status = inet_pton(AF_INET, ip, &serverSockAddr.sin_addr);
    serverSockAddr.sin_port = htons(port);
    if(status != 1)
    {
        return false;
...
bool Client::connectToServerWithRawIP(const char* ip, int port) {
    settingIPAndPort(ip, port);
    if (!setupSocket())
    {
        return false;
...
void Client::recvAppendToSSUntilEnoughBytes(int bytes) {
    ...
    std::cout << "Received: " << bytesCount << " bytes" << std::endl;
    ss << std::string(msgBuf);
}
